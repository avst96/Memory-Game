...
(function (gameTs) {
    ...
    function doMove(event) {
        ...
        if (isGameSolo && !prevPickedCards.includes(card)) {
            prevPickedCards.push(card);
        }
        if ($('.picked').length === 2) {
    ...
    function setGameStatusAndMsg() {
        if ($('.claimed').length === 20) {
            ...
            if (player1Score === player2Score) {
                ...
                currentTurn = player1Score > player2Score ? 3 : isGameSolo ? 6 : 4;
            }
        ...
        ;
        if (gameStatus) {
            currentTurn = currentTurn === 1 || currentTurn === 2 ? 0 : isGameSolo ? 2 : 1;
        }
        ;
        setTurnMsg();
        if (currentTurn === 2) {
        ...
        if (gameStatus) {
            if (!pickedCardsMatch()) {
                ...
                prevPickedCards.forEach(c => {
                    if (c.innerHTML === comPick1.innerHTML) {
                        comPick2 = c;
                    }
                });
                comPick2 = comPick2 || unclaimedCards[getRndNum(unclaimedCards.length)];
                while (comPick2 === comPick1) {
                    comPick2 = unclaimedCards[getRndNum(unclaimedCards.length)];
                }
            }
            setTimeout(() => {
                comPick1.click();
